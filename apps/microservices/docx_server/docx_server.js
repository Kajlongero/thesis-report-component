...
const packageDefinition = protoLoader.loadSync(
  PROTO_PATH_DEFINITIONS.docx.path,
  {
    keepCase: true,
    longs: String,
    enums: String,
...
const docxProto = grpc.loadPackageDefinition(packageDefinition).docx;

function main() {
  const server = new grpc.Server();

  server.addService(docxProto.ReportService.service, {
    GenerateReport: GenerateReports,
  });

  const port = PROTO_PATH_DEFINITIONS.docx.port;
